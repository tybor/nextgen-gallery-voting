...
* There is a conflict with a NGG bug that breaks voting if you are using the `[nggtags]` shortcode. For now, you can work around the bug by adding the voting tag (`<?php echo nggv_imageVoteForm($image->pid); ?>`) to the `/nextgen-gallery/view/imagebrowser.php` template.
